...
### 思维导图：Harness Engineering 与 Ralph Loop 关系
...
```
           ...
           ▼                              ▼                                ▼
┌─────────────────────┐      ┌─────────────────────┐      ┌─────────────────────┐
│ Harness Engineering│      │      互补关系        │      │    Ralph Loop       │
│    (系统架构层)      │◄────►│                      ├─────►│    (方法论层)        │
└─────────────────────┘      └─────────────────────┘      └─────────────────────┘
           │                                                          │
           ├────────────────────┐                      ┌─────────────┴────────────┐
           ▼                    ▼                      ▼                        ▼
    ┌─────────────┐      ┌─────────────┐      ┌─────────────────┐      ┌─────────────────┐
    │  引导       │      │  传感器      │      │  原子任务分解    │      │  通过/失败标准   │
    │ (Guides)   │      │ (Sensors)   │      │                 │      │                 │
    └─────────────┘      └─────────────┘      └─────────────────┘      └─────────────────┘
           │                    │                       │                        │
           ├─计算型            ├─计算型              ├─最小可验证单元          ├─客观可测试条件
           │  -LSP            │  -linter            │  -独立执行              │  -明确判定标准
           │  -CLI            │  -测试              │  -单一职责              │  -二进制结果
           │  -codemods      │  -类型检查          │  -清晰边界              │  -自动化验证
           │
           ├─推理型            ├─推理型
           │  -AGENTS.md      │  -AI代码审查
           │  -Skills         │  -架构审查
...
```
          ...
          ▼                                                           ▼
┌─────────────────────────┐                               ┌─────────────────────────┐
│      Ralph Loop 中的引导 │                               │    Ralph Loop 中的传感器 │
│      (Guides)           │                               │      (Sensors)          │
└─────────────────────────┘                               └─────────────────────────┘
          │                                                           │
          ├─ RALPH.md 任务定义                                      ├─ 测试脚本
